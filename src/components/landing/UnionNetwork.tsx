 ...
 * a node names it with its establishment year; tapping bursts pulses.
 *
...
const NODES: [code: string, name: string, est: number | null][] = [
  ["AP", "Andhra Pradesh", 1956], ["AR", "Arunachal Pradesh", 1987], ["AS", "Assam", 1950], ["BR", "Bihar", 1950],
  ...
  ["MZ", "Mizoram", 1987], ["NL", "Nagaland", 1963], ["OD", "Odisha", 1950], ["PB", "Punjab", 1966],
  ["RJ", "Rajasthan", 1956], ["SK", "Sikkim", 1975], ["TN", "Tamil Nadu", 1950], ["TS", "Telangana", 2014],
  ["TR", "Tripura", 1972], ["UP", "Uttar Pradesh", 1950], ["UK", "Uttarakhand", 2000], ["WB", "West Bengal", 1950],
  ["AN", "Andaman & Nicobar", 1956], ["CH", "Chandigarh", 1966], ["DD", "DNH & Daman-Diu", 2020], ["DL", "Delhi", null],
  ["JK", "Jammu & Kashmir", null], ["LA", "Ladakh", 2019], ["LD", "Lakshadweep", 1956], ["PY", "Puducherry", 1963],
];
    ...
    <section className="lsec-tint">
      <div className="net-head">
        ...
        <p className="lsub">
          ...
          flows into the same archive. Hover a node to see when it took its present form.
        </p>
      </div>
      <UnionNetwork />
  ...
  useEffect(() => {
    ...
    const draw = (dt: number) => {
      ...
      if (hov >= 0) {
        ...
        // Establishment dates per the archive's own states sheet (formed_on);
        // Delhi and J&K predate their present arrangements and carry no year.
        const label = d[2] === null ? d[1].toUpperCase() : `${d[1].toUpperCase()} · EST. ${d[2]}`;
        ...
        ctx.beginPath();
        // Plain roundRect: every browser new enough for the site's color-mix
        // styling has it, and the feature-test narrowed the type to never.
        ctx.roundRect(bx, by, tw, th, 9 * S);
